...
m = length(b); opts = FPC_AS_opts(opts,n,m);
...
if mu >= normAtMb; x = zeros(n,1); Out.crit2 = 0; Out.exit = 1; Out.mesg = 'zero is optimal'; final_solu_info();  return; end


tauD = opts.tauD; tau = tauD; muf = mu;
...
if nnz_x <= m && crit2f < opts.gtol ;
    ...
    mu_prev = normAtMb;    mu = mu_prev*opts.eta;
end
f=calc_f(); fp = f;
